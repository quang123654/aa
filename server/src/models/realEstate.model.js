...
RealEstate.updateRealEstate = (RealEstateId, realEstateReqData, result) => {
  dbConn.query(
    "UPDATE realestate SET Title=?,RoomCode=?, Price=?, Location=?, Address=?, Direct=?, Floor=?, imgURL=?, Area=?, Toilet=?, People=?, Detail=?, Building=?, CreateAt=?, datesBooked=?  ,UserId=? WHERE RealEstateId= ?",
    [
