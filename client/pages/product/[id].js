...
export default function Product() {
  ...
  const diff = Math.abs(date[0].endDate.getTime() - date[0].startDate.getTime());
  const noofdays = Math.ceil(diff / (1000 * 3600 * 24));
  // console.log(noofdays)

  // let data = {
  //   RealEstateTitle: pdList[0].Title,
  //   RoomCode: pdList[0].RoomCode,
  //   Address: pdList[0].Address,
  //   imgURL: pdList[0].imgURL,
  //   Price: pdList[0].Price * noofdays,
  //   DatesBooked: arrDate,
  //   ethAddress: address
  // }
  ...
  async function rentRealEstate(nft) {
    ...
    const pricePerDate = ethers.utils.parseUnits(nft.Price.toString(), "ether");
    console.log(typeof pricePerDate)

    const transaction = await contract.addDatesBooked(nftaddress, nft.tokenId, arrDate, { value: pricePerDate * noofdays })
    await transaction.wait();
    router.push("/");
  }

  // const createData = (nft) => {
  //   Axios.post(`http://localhost:5000/api/datebooking/`, data).then((rs) => {
  //     rentRealEstate(nft)
  //     console.log(nft)
  //   }).catch((err) => {
  //     console.log(err)
              ...
              <div className="w-full max-w-6xl flex flex-col gap-2 relative">
                ...
                <div className="flex">
                  ...
                  <div className="p-4 shadow-[0_1px_4px_rgba(0,0,0,0.16)] bg-yellow-200 w-1/3 ">
                    ...
                    <button className=" w-full text-white p-2  bg-cyan-500 rounded hover:bg-cyan-600" onClick={rentRealEstate(each)}
                    >
                      Reserve or Book Now!
                    </button>
                  </div>
